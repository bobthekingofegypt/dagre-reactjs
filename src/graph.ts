...
export class Graph {
  ...
  graphEdge(edge: Edge): EdgeOptions {
    return this.graph.edge(edge) as EdgeOptions;
  }

  graphEdgeByIds(to: string, from: string, name?: string): GraphEdge {
    return this.graph.edge(to, from, name);
  }
